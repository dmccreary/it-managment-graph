---
name: it-graph-generator
description: This skill generates interactive graph-model visualizations using vis-network library for IT management educational content. Use this skill when users request creating interactive IT graph visualizations, network diagrams, dependency graphs, or node-based microsims for educational textbooks. The skill creates complete microsim packages with HTML, JavaScript, CSS, and documentation that can be embedded in MkDocs chapters via iframes.
---
...
This skill generates complete, interactive IT graph visualizations (MicroSims) for educational content. Each microsim uses the vis-network JavaScript library to create force-directed network visualizations with features like search, filtering, hover tooltips, and interactive node details. The skill is optimized for IT management education, particularly for graduate-level courses on graph databases and IT infrastructure modeling.

## When to Use This Skill

Invoke this skill when:

- User requests creating an interactive IT graph visualization or network diagram
- User asks to generate a microsim showing nodes, relationships, or dependencies
- User wants to visualize IT infrastructure, business services, or system architectures
- User mentions creating educational visualizations with vis-network
- User references the examples in `/docs/prompts/create-it-graph-generator-skill.md`
...
6. **Generate Documentation** - Create index.md with educational content
7. **Validate Output** - Check data integrity and file structure
8. **Report Results** - Provide summary with file paths and next steps
...
1. **microsim_name** (kebab-case) - Directory name (e.g., "it-asset-nodes", "dependency-graph")
2. **title** - Human-readable title (e.g., "IT Asset Relationship Graph")
...
```javascript
{
  type: "BusinessService",           // Type name (PascalCase)
  shape: "circle",                    // Vis-network shape
  color: "#ec4899",                   // Hex color code
  size: 40,                           // Size in pixels (25-40)
  icon: "../../it-icons/service.svg", // Optional icon path
  properties: ["name", "SLA_tier", "owner", "uptime_target"]  // Example properties
}
```
...
{
  id: 1,                              // Unique integer
  label: "Customer Portal",           // Display name
  type: "BusinessService",            // Matches node type
...
Before assigning icons, check if they exist in `/docs/it-icons/`:

- Use Glob tool to list available icons: `pattern: "it-icons/*.svg"`
- Assign appropriate icons to node types based on semantics
...
- `{{SUBTITLE}}` - Subtitle or default "Interactive IT Graph Visualization"
...
```html
<div class="callout-message">Each node represents a unique entity in your IT infrastructure</div>
...
```markdown
- **BusinessService** (Pink, Circle): Represents high-level business services
  - Properties: name, SLA_tier, owner, uptime_target, users
- **Application** (Light Blue, Box): Software applications
  - Properties: name, technology, version, environment, team
```
...
```
✅ Successfully generated IT graph microsim: <title>
...
"Interactive IT Graph Visualization"

### Standard Node Colors

Refer to `references/ui-specifications.md` for the complete color palette. Common types:
...
This skill includes comprehensive resources for generating IT graph microsims:
...
Always check `/docs/it-icons/` directory first using the Glob tool:

```
pattern: "it-icons/*.svg"
```

Use icons when available and semantically appropriate. Fall back to shapes when:
...
- Use meaningful relationship labels (e.g., "DEPENDS_ON", "PROVIDES", "MANAGES")
...
**Solution**: Verify icon paths are correct (../../it-icons/<name>.svg) and icons exist
